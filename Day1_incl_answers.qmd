...
The code chunks throughout the tutorial are designed to lead you from quality cleaning to DEG analysis. To better understand the workflow and avoid errors, it is important to explore the outputs of each command further. 
...
**The conda environment can be activated with the following commands**:
...
If you want to install software yourself, you can find the command to install your own conda environment below:
...
In transcriptomics it is important to know, which strand was sequenced as this affects downstream steps. You may encounter the following terminology when exploring the manual of different software tools:
...
3.  We work with reverse stranded sequencing data. In the methods section these two sections tell you more about this:
...
To reduce compute time, we won't be working with the full dataset but a sub-sampled dataset that was already prepared for you (\~10 million reads/sample). The data are available here:  
...
4.  Based on the [sample information on ENA](https://www.ebi.ac.uk/ena/browser/view/PRJEB12372) generate a mapping file, i.e. a file that links the sample IDs with more descriptive information:
    ...
    -   Generate a metadata file called `metadata.txt` in your project directory. Add two tab-separated headers: runID and treatment. Then add 6 rows with the sample information (3 samples from treated and 3 samples from untreated conditions). 
    - Hint: To have a consistent metadata table, use the term "treated" samples from 400 ppm and "untreated", for samples grown under 1200 ppm.
...
2.  On the linked page we can find the assembly as FASTA file that can be accessed by clicking `SET FASTA` on for example the page for [HADH01000001](https://www.ebi.ac.uk/ena/browser/view/HADH01000001). However, finding the raw FASTQ data is more bit difficult. For example, if we click on `Sequence versions: View` we see that we can not actually download any raw data.
...
```{bash}
# Generate a project directory in ~/personal and call it `denovo`
...

# Set an environmental variable to the folder with the raw data 
# Note: It is recommended to store all custom paths as variables at the top of your code, 
# then you only have only location where you have to change the code later
DATA="/zfs/omics/projects/education/BBDA_transcriptomics/denovo/data"

# Make a folder to store the fastqc results
mkdir -p quality/raw

# Run FastQC
# Hints: run FastQC on all files at once
#       ensure that the data is generated in the output folder that you have just generated
#       submit this on the compute node with srun with these settings:
#       `--cpus-per-task 5` and `--mem=20G`
#       Make use of the DATA variable to access the raw fastq.gz files
srun --cpus-per-task 5 --mem=20G ...

# Run seqkit
# `seqkit stats` can be used to generate summary stats for multiple fastq files in a single text file
# Hints:   Use srun with 5 CPUs and 20G of memory, 
#          generate a single file with all stats for all 12 fastq files in tabular format
...
# Step 1: seqkit stats output has the number of reads in column 4
# Step 2: Use awk to:
#         - skip the header row (NR>1)
#         - sum the values from column 4 (sum+=$4)
# Step 3: Print the total number of reads at the end
...
# Download the FastQC HTML files to your personal computer
# Hint: When using scp run the scp command from your local computer not the HPC
...
# Run FastQC
srun --cpus-per-task 5 --mem=20G fastqc ${DATA}/*fastq.gz -o quality/raw --threads 10

# Run seqkit
srun --cpus-per-task 5 --mem=20G seqkit stats ${DATA}/*fastq.gz -aTo quality/raw/seqkit.tsv --threads 5

# Answer: We work with 60,000,000 reads
awk 'NR>1 {sum+=$4} END {print sum}' quality/raw/seqkit.tsv

# Download the FastQC HTML files
scp user@omics-h0.science.uva.nl:/home/user/personal/denov/quality/raw/*html quality/raw
```
:::
...
There is nothing unusual. The sequence quality is good over the whole length of the sequence with a small drop in quality at the end of the sequence, however, that is nothing unusual for Illumina data. We see a very small fraction of Illumina adapter sequence, which me might want to clean.
:::
:::
...
```{bash}
# Make suitable folders for the outputs 
# Also make folders to store sbatch scripts and log files to keep things organized
...
...
# Extract the sample ID from metadata.txt and use it for the array
# - cut: takes the first column in metadata.txt (sample IDs)
...
# Run fastp
# - Use the DATA and SAMPLE variables to build file names
...
fastp -i ${DATA}/${SAMPLE}_1.fastq.gz -I ${DATA}/${SAMPLE}_2.fastq.gz \
    -o fastp/${SAMPLE}_1_paired.fastq.gz -O fastp/${SAMPLE}_2_paired.fastq.gz \
    --unpaired1 fastp/${SAMPLE}_1_unpaired.fastq.gz --unpaired2 fastp/${SAMPLE}_2_unpaired.fastq.gz \
...
# Run FastQC
srun --cpus-per-task=5 --mem=20G fastqc fastp/*_paired.fastq.gz -o quality/cleaned --threads 5

# Run SeqKit
srun --cpus-per-task=5 --mem=20G seqkit stats fastp/*_paired.fastq.gz -aTo quality/cleaned/seqkit.tsv --threads 5
...
The cleaning worked, the quality is slightly better and the adapters sequences are gone. We now still work with \~97% of the data, which is in a very good range.
:::
:::

## Assembly

There are a range of different tools for de novo assembly. In this tutorial, we will use Trinity [@Grabherr2011], an open source tool that comes with a long list of associated tools for downstream-analyses.

```{bash}
# Create a samples mapping file suited Trinity
# Step 1: Check the Trinity documentation for the expected format:
#         sample_name   replicate_name   read1_path   read2_path
# Step 2: Use awk to generate this mapping automatically from metadata.txt
...
# Step 3: Adjust paths if your FastP output folder has a different name
awk 'BEGIN{OFS="\t"} 
        NR>1 {
            printf "%s\t%s_rep%d\tfastp/%s_1_paired.fastq.gz\tfastp/%s_2_paired.fastq.gz\n", \
            $2, $2, ++count[$2], $1, $1
        }' metadata.txt > trinity_mapping.txt
...
#        - `source ~/.bashrc` is needed to activate conda inside compute node
...
# Run trinity
Trinity --seqType fq --max_memory 50G --samples_file trinity/trinity_mapping.txt \
    --SS_lib_type RF  --inchworm_cpu 20 --CPU 20 \
